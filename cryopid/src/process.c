...
/* Insert an lseek system call into the target, execute it, and pretend it
 * never happened.  Bwahahahaha.
 */
off_t vampire_lseek(pid_t tpid, int fd, off_t offset, int whence)
{
...
struct proc_image_t* get_proc_image(pid_t target_pid, int flags) {
	...
	memset(proc_image->dir, 0, sizeof(proc_image->dir));
	readlink(tmp_fn, proc_image->dir, sizeof(proc_image->dir)-1);
	...
	for (;;) {
		...
		if (S_ISREG(stat_buf.st_mode)) {
			file_offset = vampire_lseek(target_pid, proc_image->fds[fd_count].fd, 0, SEEK_CUR);

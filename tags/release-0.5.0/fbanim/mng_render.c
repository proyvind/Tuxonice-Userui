...
static struct fbanim_mng_data mng_data;

extern mng_ptr fbanim_mng_memalloc(mng_size_t len);
extern void fbanim_mng_memfree(mng_ptr p, mng_size_t len);
...
int mng_init(char* filename, int bytes_pp) {
	...
	mngh = mng_initialize(&mng_data, fbanim_mng_memalloc, fbanim_mng_memfree,
			MNG_NULL);
	if (mngh == MNG_NULL)
		return 0;
